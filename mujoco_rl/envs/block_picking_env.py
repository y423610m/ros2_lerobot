...
class BlockPickingEnv(MujocoEnv):
    ...
    def _update_phase(
        ...
        if p == Phase.APPROACH:
            ...
            if is_object_above_near_container:
                self._phase = Phase.RELEASE
        elif p == Phase.RELEASE:
            if not is_gripper_touching or not is_finger_touching:
    ...
    def _compute_reward_phased(self, action: np.ndarray) -> tuple[float, dict]:
        ...
        if self._reward_type == "sparse":
            ...
            reward = (r_progress + r_reach + r_touch_gripper + r_touch_finger + r_touch
                      ...
                      + r_placement_success + r_home + r_success + r_alive + r_ctrl
    ...
    def _compute_reward_v2(self, action: np.ndarray) -> tuple[float, dict]:
        ee_gripper = self.data.site_xpos[self._ee_gripper_sid].copy()
        ee_wrist = self.data.site_xpos[self._ee_wrist_sid].copy()
        ee_pos = (ee_gripper + ee_wrist) / 2.0
        block_pos = self.data.site_xpos[self._block_sid].copy()
        ...
        if self._reward_type == "sparse":
            ...
            elif is_object_above_container:
                # Above container -> short drop: reward EE near the rim and gripper open.
                ee_above_rim = max(0.0, float(ee_pos[2] - (TABLE_Z + CONTAINER_RIM_HEIGHT)))
                r_transport = (0.3 * (-d_block_target_xy) + np.exp(-20 * d_block_target_xy)) * REWARD_WEIGHTS["transport"]
                r_descend   = np.exp(-20 * ee_above_rim) * REWARD_WEIGHTS["descend"] * 5.0
                r_release   = float(
            ...
            reward = (r_reach + r_transport + r_lift + r_descend + r_release + r_placement
                      + r_home + r_ee_height
    ...
    def _compute_reward(self, action: np.ndarray) -> tuple[float, dict]:
        ...
        if self._reward_type == "sparse":
            ...
            r_ee_height = 0.0
            r_alive = 0.0
            r_container_touch = 0.0
            r_container_move = 0.0
